...
| `watermark` | What a process did. Links the others. | its process ending |
...
decides it — which statistic, under what conditions. A goal with no observation
against it is *unmeasured*, which is not the same as unmet. Work that finishes
is an item on the state surface; a goal is not work.

Which atom writes which kind, so no atom has to decide twice:
