...
is also how scope collisions get prevented: where two slices would edit the same
files, either draw the blocking edge or merge them. Leaving both open and
unblocked promises a parallelism that resolves as a merge conflict. The rule
that makes it necessary is one writer per tree — see
